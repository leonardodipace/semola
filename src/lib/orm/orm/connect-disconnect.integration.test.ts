  ...
  describe(`${live.adapter} connect and disconnect mutations`, () => {
    ...
    describe("edge cases", () => {
      ...
      test("swaps post links via connect and disconnect in same hasMany update", async () => {
        ...
        const oldPost = await orm.posts.findUnique({
          where: { id: edgePostId },
        });
        const newPost = await orm.posts.findUnique({
          where: { id: edgePostId2 },
        });
      ...
      test("connects to-many when select omits the primary key", async () => {
        const orm = await open();
      ...
      test("connects to-one when parent is updated by unique non-primary key", async () => {
        const orm = await open();
        ...
        await orm.users.create({
          data: { id: userId2, email: "rival@example.com" },
        });
        await orm.users.create({
          data: { id: userId, email: "owner@example.com" },
        });
