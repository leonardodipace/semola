...
const schemaSql = {
  sqlite: {
    ...
    meta: "CREATE TABLE meta (id TEXT PRIMARY KEY, payload TEXT NOT NULL, settings TEXT NOT NULL)",
    insertUser:
      "INSERT INTO users (id, first_name, created_at, is_active) VALUES (?, ?, ?, ?)",
    insertPost: "INSERT INTO posts (id, title, author_id) VALUES (?, ?, ?)",
  },
  postgres: {
    users:
      "CREATE TABLE users (id TEXT PRIMARY KEY, first_name TEXT NOT NULL, created_at TIMESTAMP NOT NULL, is_active BOOLEAN NOT NULL)",
    posts:
      "CREATE TABLE posts (id TEXT PRIMARY KEY, title TEXT NOT NULL, author_id TEXT NOT NULL)",
    meta: "CREATE TABLE meta (id TEXT PRIMARY KEY, payload JSONB NOT NULL, settings JSONB NOT NULL)",
    insertUser:
      "INSERT INTO users (id, first_name, created_at, is_active) VALUES ($1, $2, $3, $4)",
    insertPost: "INSERT INTO posts (id, title, author_id) VALUES ($1, $2, $3)",
  },
} as const;

for (const live of integrationAdapters()) {
  describe(`${live.adapter} dialect integration`, () => {
    const ddl = schemaSql[live.adapter];

    const setupUsers = async (sql: Bun.SQL) => {
      await live.beforeEach?.();
      await sql.unsafe(ddl.users);
    };

    const setupUsersAndPosts = async (sql: Bun.SQL) => {
      await setupUsers(sql);
      await sql.unsafe(ddl.posts);
    };

    const setupMeta = async (sql: Bun.SQL) => {
      await live.beforeEach?.();
      await sql.unsafe(ddl.meta);
    };
    ...
    const insertUser = async (
      ...
      await sql.unsafe(ddl.insertUser, [id, firstName, createdAt, isActive]);
    };
