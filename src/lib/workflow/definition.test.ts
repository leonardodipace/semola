...
import { clearWorkflowRegistry } from "./index.js";
import {
  createRedis,
...
describe("workflow", () => {
  ...
  test("starts workflow with numeric id", async () => {
    const redis = createRedis();
    const workflow = createWorkflowWithEchoResult("run-matrix", redis);

    await workflow.start({ id: 7 }, { executionId: "run-matrix-exec" });
    const execution = await waitForExecution(workflow, "run-matrix-exec");

    expect(execution.result).toBe("echo:7");
  });

  test("does not re-run completed execution on resume", async () => {
    const redis = createRedis();
    const calls = { value: 0 };
    ...
    await workflow.start({ id: 3 }, { executionId: "resume-matrix-exec" });
    await waitForStatus(workflow, "resume-matrix-exec", "completed");

    const resumeData = await workflow.resume("resume-matrix-exec");
    const execution = await workflow.get("resume-matrix-exec");

    expect(resumeData.status).toBe("completed");
    expect(execution.result).toBe("echo:3");
    expect(calls.value).toBe(1);
  });

  test("rejects duplicate execution id", async () => {
    const redis = createRedis();
    const workflow = createWorkflowWithEchoResult("dupe-matrix", redis);

    await workflow.start({ id: 1 }, { executionId: "dupe-matrix-exec" });

    await expect(
      workflow.start({ id: 2 }, { executionId: "dupe-matrix-exec" }),
