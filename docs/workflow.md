...
- **`retentionTTL`** - how long terminal executions (`completed` / `failed` / `cancelled`) stay in Redis, in ms (default: 86400000, 24h). `0` unlinks immediately after terminal. Pending and running keys are never expired. Failed executions can be `resume`d only while they still exist. A background sweep also expires leftover terminal keys with no TTL (including data from older semola versions).
- **`retentionMax`** - optional cap on terminal executions per workflow name. Oldest are `UNLINK`ed when the cap is exceeded.
