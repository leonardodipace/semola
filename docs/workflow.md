...
- **`timeout`** - per-step timeout in ms (default: same as `lockTTL`). Must be a positive finite number or `Infinity`. A hang is a retryable step failure, not a stuck `running` execution. The step `signal` is aborted; the execution is not cancelled, so retries can run. Handlers that ignore `signal` may keep running after timeout. Pass `Infinity` only if a step is allowed to run longer than the lease.
