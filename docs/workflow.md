...
- **`timeout`** - per-step timeout in ms (default: same as `lockTTL`). A hang is a retryable step failure, not a stuck `running` execution. Pass `Infinity` only if a step is allowed to run longer than the lease.
