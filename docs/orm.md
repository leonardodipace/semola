...
- Review `up.sql` / `down.sql` before `apply`, especially drop warnings.
- Ambiguous drop+add asks whether it is a rename or create-from-scratch (TTY).
- Destructive drops ask for confirmation.
- Do not hand-edit applied migration files; keep folders in order.
